...
class Romanamphitheatre extends StatefulWidget {
  ...
  @override
  State<Romanamphitheatre> createState() => _RomanamphitheatreState();
}

class _RomanamphitheatreState extends State<Romanamphitheatre> {
  ...
  void _addReview() async {
    if (_reviewController.text.isNotEmpty) {
      User? user = FirebaseAuth.instance.currentUser;
      if (user != null) {
        await _firestore.collection('romanamphitheatre_reviews').add({
  ...
  void _updateReview(String reviewId) async {
    if (_reviewController.text.isNotEmpty) {
      await _firestore.collection('romanamphitheatre_reviews').doc(reviewId).update({
        'review': _reviewController.text,
        'timestamp': FieldValue.serverTimestamp(),
      });
      _reviewController.clear();
  ...
  void _deleteReview(String reviewId) async {
    User? user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      await _firestore.collection('romanamphitheatre_reviews').doc(reviewId).delete();
    }
  }
  ...
  void _toggleLike(String reviewId, List likes) async {
    ...
    if (user != null) {
      ...
      if (isLiked) {
        _firestore.collection('romanamphitheatre_reviews').doc(reviewId).update({
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            const Text(
                              ...
                              "332 B.C on the site of an old fishing village. It has\nthrived"
                              " for more than 500 years and was the capital\ncity"
