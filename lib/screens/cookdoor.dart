...
class CookDoor extends StatefulWidget {
  ...
  @override
  State<CookDoor> createState() => _CookDoorState();
}

class _CookDoorState extends State<CookDoor> {
  ...
  void _addReview() async {
    if (_reviewController.text.isNotEmpty) {
      ...
      if (user != null) {
        ...
        await _firestore.collection('CookDoor_reviews').add({
  ...
  void _updateReview(String reviewId) async {
    if (_reviewController.text.isNotEmpty) {
      await _firestore.collection('CookDoor_reviews').doc(reviewId).update({
        'review': _reviewController.text,
        'timestamp': FieldValue.serverTimestamp(),
      });
      _reviewController.clear();
  ...
  void _deleteReview(String reviewId) async {
    User? user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      await _firestore.collection('CookDoor_reviews').doc(reviewId).delete();
    }
  }
  ...
  void _toggleLike(String reviewId, List likes) async {
    ...
    if (user != null) {
      ...
      if (isLiked) {
        _firestore.collection('CookDoor_reviews').doc(reviewId).update({
  ...
  Widget build(BuildContext context) {
    final Uri toLaunch = Uri(scheme: 'https', host: 'www.facebook.com', path: '/cookdooreg/');
