...
class Zeeyara extends StatefulWidget {
  const Zeeyara({super.key});

  @override
  State<Zeeyara> createState() => _ZeeyaraState();
}

class _ZeeyaraState extends State<Zeeyara> {
  ...
  void _addReview() async {
    if (_reviewController.text.isNotEmpty) {
      ...
      if (user != null) {
        ...
        await _firestore.collection('Zeeyara_reviews').add({
  ...
  void _updateReview(String reviewId) async {
    if (_reviewController.text.isNotEmpty) {
      await _firestore.collection('Zeeyara_reviews').doc(reviewId).update({
        'review': _reviewController.text,
        'timestamp': FieldValue.serverTimestamp(),
      });
      _reviewController.clear();
  ...
  void _deleteReview(String reviewId) async {
    User? user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      await _firestore.collection('Zeeyara_reviews').doc(reviewId).delete();
    }
  }
  ...
  void _toggleLike(String reviewId, List likes) async {
    ...
    if (user != null) {
      ...
      if (isLiked) {
        _firestore.collection('Zeeyara_reviews').doc(reviewId).update({
  ...
  Widget build(BuildContext context) {
    final Uri toLaunch = Uri(scheme: 'https', host: 'web.facebook.com', path: '/zeeyara.restaurant');
                        ...
                        Container(
                          decoration: BoxDecoration(
                            ...
                            image: const DecorationImage(
                              image: AssetImage("assets/zeeyara-pyramids-elite.jpg"),
                              fit: BoxFit.cover,
                            ),
                          ),
                          width: 341,
                      ...
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            const Text(
                              "“Tucked away in the heart of Islamic Old Cairo,\n"
                              "Zeeyara provides traditional Egyptian cuisine elevated\n"
